...
export function usePageDrop(onFile, disabled = false) {
  ...
  const onDrop = useCallback(async e => {
    ...
    // Collect ALL valid PDF/DOCX/TXT files from the drop
    ...
    // Pass array — uploadDoc handles sequential queuing
    if (picked.length === 1) await onFile?.(picked[0])
    else if (picked.length > 1) await onFile?.(picked)
  }, [disabled, onFile])
